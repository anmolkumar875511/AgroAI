...
function deterministicOffset(seed: number, axis: number) {
  const raw = Math.sin(seed * 12.9898 + axis * 78.233) * 43758.5453;
  return (raw - Math.floor(raw) - 0.5) * 0.1;
}
  ...
  const heatmapData = useMemo(() => {
    ...
    pestOutbreaks.forEach((outbreak) => {
      ...
      for (let i = 0; i < pointCount; i++) {
        const latOffset = deterministicOffset(outbreak.id * 100 + i, 1);
        const lngOffset = deterministicOffset(outbreak.id * 100 + i, 2);
