...
export function PriorityCard({ visit, retailerId, territoryId }: PriorityCardProps) {
  ...
  return (
    <>
      <div className="bg-[#1E293B] rounded-xl border border-white/10 hover:border-[#1976D2]/25 transition-all duration-300">
        <div className="p-5 flex flex-col lg:flex-row gap-5 items-start">
          ...
          <div className="flex-shrink-0 self-center lg:self-start">
            <ProgressRing progress={visit.score} size={100} strokeWidth={7} color="url(#ringGradient)" animate>
              <div className="text-center">
                <div className="text-2xl font-extrabold text-[#8BC34A]">{visit.score}</div>
                <div className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Priority Score</div>
              </div>
            </ProgressRing>
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-lg font-semibold text-white">{visit.name}</h4>
            ...
            <div className="mt-4 p-3 rounded-lg bg-[#0F172A]/60 border border-[#1976D2]/25 border-l-[3px] border-l-[#388E3C]">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-[#1976D2] mb-1">Reason</p>
              <div className="flex items-start gap-2">
                <Sparkles className="w-4 h-4 text-[#8BC34A] flex-shrink-0 mt-0.5" />
          ...
          <div className="flex lg:flex-col gap-2 flex-shrink-0 w-full lg:w-auto">
            <button onClick={handlePrimaryAction} disabled={actionState !== 'idle'}
              className={cn(
                ...
                actionState === 'completed' ? 'bg-[#388E3C] text-white'
                  : 'bg-[#2E7D32] text-white hover:bg-[#276b2a] border border-[#388E3C]/40',
            ...
            <button onClick={handleSecondaryAction}
              className="flex-1 lg:flex-none px-5 py-2.5 rounded-button bg-light-gray dark:bg-white/5 text-white text-sm font-medium hover:bg-light-gray/80 transition-all">
              {visit.actions[1]}
            </button>
          </div>
        </div>
          ...
          <div className="bg-white dark:bg-deep-forest w-full max-w-lg rounded-2xl shadow-xl overflow-hidden border border-light-gray dark:border-white/10 animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-light-gray dark:border-white/10 flex justify-between items-center bg-off-white dark:bg-white/5">
              ...
              <button onClick={() => setIsModalOpen(false)}
                className="p-1.5 rounded-full hover:bg-light-gray dark:hover:bg-white/10 text-slate-400 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-5">
              <div className="flex items-center gap-4 pb-4 border-b border-light-gray dark:border-white/5">
                ...
                <div>
                  <h4 className="text-xl font-bold text-white">{visit.name}</h4>
              ...
              <div className="space-y-4">
                <div className="bg-light-gray/30 dark:bg-white/5 p-4 rounded-xl">
                  <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">AI Insight</div>
                  <div className="text-sm font-medium text-white">{visit.aiReason}</div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-light-gray/30 dark:bg-white/5 p-3 rounded-xl">
                    <div className="text-xs text-slate-400 mb-1">Last Interaction</div>
                    <div className="text-sm font-semibold text-white">{visit.lastVisit}</div>
                  </div>
                </div>
              </div>
              <div className="pt-4 flex gap-3">
                <button onClick={() => setIsModalOpen(false)}
                  className="flex-1 py-2.5 rounded-lg bg-light-gray dark:bg-white/5 text-white font-medium hover:bg-light-gray/80 transition-colors">
                  Cancel
                </button>
