...
from app.models.models import Visit, User, Recommendation, Retailer, RetailerInventory, VisitFeedback
...
async def manager_dashboard(
    ...
    for agent in agents:
        ...
        reps.append(RepSummary(
            id=str(agent.id),
            name=agent.name,
            territory=agent.territory or "Unassigned",
            visits=total_v,
            target=target,
            revenue=rev,
...
async def team_tracking(
    ...
    for agent in agents:
        ...
        reps_live.append({
            "id": agent.id,
            "name": agent.name,
            "territory": agent.territory or "Bihar Zone",
