    ...
    <div className="backdrop-blur-md bg-white/80 dark:bg-[#121b14]/40 rounded-2xl shadow-md border border-white/30 dark:border-white/5 h-full flex flex-col overflow-hidden">
      ...
      <div className="p-3.5 space-y-3 flex-1 overflow-y-auto">
        {loading && (!recs || recs.length === 0) ? (
          ...
          <div className="h-full min-h-[220px] flex flex-col items-center justify-center text-center p-6 space-y-4 animate-fade-in">
            <div className="w-12 h-12 rounded-full bg-deep-green/10 dark:bg-lime-green/10 flex items-center justify-center shadow-inner">
              <Leaf className="w-6 h-6 text-deep-green dark:text-lime-green animate-pulse-slow" />
            </div>
            <div className="space-y-1 max-w-[280px]">
              <p className="text-sm font-semibold text-text-primary dark:text-white">All Clear!</p>
              <p className="text-xs text-text-muted dark:text-white/60 leading-relaxed">
                No critical alerts or AI recommendations for this territory at this time.
              </p>
            </div>
          </div>
        ) : (
